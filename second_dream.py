...
def dream(model, loss_layers, base_umb_timeser,
    ...
    dream_b_timeser = np.copy(base_b_timeser)
    ...
    original_shape = dream_b_timeser.shape[1]
    ...
    for shape in successive_shapes:
        ...
        dream_b_timeser = _resize_b_timeser(dream_b_timeser, shape)

        # Run gradient ascent on the dream time series, doing the actual dreaming.
        _gradient_ascent(fetch_loss_and_grads, weights_tensors, dream_b_timeser, iterations, step, max_loss, verbose)
    ...
    if base_timeser_format == "u":
        dream_umb_timeser = dream_b_timeser[0].reshape(-1, order="F")
    elif base_timeser_format == "m":
        dream_umb_timeser = dream_b_timeser[0]
    elif base_timeser_format == "b":
        dream_umb_timeser = dream_b_timeser

    return dream_umb_timeser
...
def _gradient_ascent(fetch_loss_and_grads, weights_tensors, dream_b_timeser, iterations, step, max_loss, verbose):
    for i in range(iterations):
        loss_value, grad_values = fetch_loss_and_grads([dream_b_timeser, *weights_tensors])
        if max_loss is not None and loss_value > max_loss:
            break
        if verbose:
            print(f"..Loss value at iteration {i}: {loss_value}")
        dream_b_timeser += step * grad_values
